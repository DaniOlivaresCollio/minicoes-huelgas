...
```{r}
# Recodficar
etiquetas_actividad <- c("Trabaja de manera remunerada con jornada completa",
...
etiquetas_actividad_cc <- paste0(1:9, ". ", etiquetas_actividad)

# sum(is.na(data$m02)) # 48 casos
data <- data %>%
  filter(!is.na(m02)) %>%
  mutate(actividad = factor(m02, 
                            levels = 1:9, 
                            labels = etiquetas_actividad_cc))
# class(data$actividad)
# levels(data$actividad)
# as.numeric(data$actividad)
...
Al remover los NA's de `actividad` se queda con `r n` observaciones.
...
Se crea la variable de actividad solamente para ocupados: `actividad2`.

```{r}
# Recodficar
etiquetas_actividad2 <- c("Trabaja de manera remunerada con jornada completa",
                          "Trabaja de manera remunerada a tiempo parcial",
                          "Estudia y trabaja")

etiquetas_actividad2_cc <- paste0(1:3, ". ", etiquetas_actividad2)

data <- data %>%
  mutate(
    actividad2 = if_else(m02 %in% 1:3, m02, NA_real_),
    actividad2 = factor(actividad2,
                        levels = 1:3,
                        labels = etiquetas_actividad2_cc
                        ))

# Tabla de frecuencias y procentajes
...
etiquetas_rel_empleo_cc <- paste0(1:7, ". ", etiquetas_rel_empleo)

data <- data %>% 
  mutate(rel_empleo = factor(m07,
                             levels = 1:7,
                             labels= etiquetas_rel_empleo_cc
                             ))
```
...
data <- data %>%
  ...
  mutate(rel_empleo = if_else(!is.na(rel_empleo), 
         ...
         rel_empleo = factor(rel_empleo,                                # Convertir de nuevo a factor
                             levels = etiquetas_rel_empleo_cc)) %>%
...
```{r}
data <- data %>%
  mutate(rel_empleo2_raw = case_when(
    as.numeric(rel_empleo) == 4 ~ 1,
    as.numeric(rel_empleo) == 5 ~ 2,
    as.numeric(rel_empleo) %in% c(1,2,3,6,7) ~ 3,
    TRUE ~ NA_real_
  ))

etiquetas_rel_empleo2 <- c("Empleador", "Autoempleado", "Asalariado")

etiquetas_rel_empleo2_cc <- paste0(1:3, ". ", etiquetas_rel_empleo2)

data <- data %>%
  mutate(rel_empleo2 = factor(rel_empleo2_raw,
                              levels = 1:3,
                              labels = etiquetas_rel_empleo2_cc)) %>%
  select(-rel_empleo2_raw)  # eliminar variable temporal si querés
