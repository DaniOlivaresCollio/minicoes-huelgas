...
```{r}
elsoc_long_2016_2022 <- elsoc_long_2016_2022 %>% filter(d04_01>999)
elsoc_long_2016_2022 <- elsoc_long_2016_2022 %>% filter(d04_01<100000000000000000)
summary(elsoc_long_2016_2022$d04_01)
elsoc_long_2016_2022 <- elsoc_long_2016_2022 %>% filter(d04_02>999)
summary(elsoc_long_2016_2022$d04_02)
elsoc_long_2016_2022$brecha_just <- log(elsoc_long_2016_2022$d04_01/elsoc_long_2016_2022$d04_02)
#elsoc_long_2016_2022 <- elsoc_long_2016_2022 %>% filter(brecha_just>=0)
summary(elsoc_long_2016_2022$brecha_just)
```
...
elsoc <- dcast(elsoc_long_2016_2022, idencuesta~ola,
               value.var = c("brecha_just", "c08_02", "m0_edad", "m0_sexo"))
```

## Select variables
```{r}
old <- c("idencuesta", paste0("brecha_just_", 1:6), paste0("c08_02_", 1:6),
         "m0_edad_1", "m0_sexo_1")
new <- c("id", paste0("brecha_just",1:6), paste0("marchar",1:6),"edad", "sexo")
...
# crear los componentes between
  ...
  RI_y =~ 1*brecha_just1 + 1*brecha_just2 + 1*brecha_just3 + 1*brecha_just4 + 1*brecha_just5 + 1*brecha_just6
  '
```
...
# Constreñir las varianzas del error de medicion a 0
  ...
  marchar5 ~~ 0*marchar5
  marchar6 ~~ 0*marchar6
  brecha_just1 ~~ 0*brecha_just1
  brecha_just2 ~~ 0*brecha_just2
  brecha_just3 ~~ 0*brecha_just3
  brecha_just4 ~~ 0*brecha_just4
  brecha_just5 ~~ 0*brecha_just5
  brecha_just6 ~~ 0*brecha_just6
  '
```
...
Ambos interceptos aleatorios tienen una varianza significativa. Esto implica que hay diferencias estables, entre individuos, respecto a sus puntajes esperados en percepción de brecha y asistencia a marchar. --> Si no fuera significativa no sería necesario estimar intercepto aleatorio, podría ser cross-lagged simple
  ...
  - La covarianza entre los interceptos aleatorios es significativa (.024, SE=0.009), lo que sugiere que los individuos que tienen una alta participación en marchas también tienen una alta percepción de brecha salarial
...
Efectos autorregresivos:
  ...
  - Solo para marchar es significativos (marchar: b=0.143; SE=0.021); para justificación de brecha salarial no es significativo (Brecha: b=0.022; SE=0.017).
...
Efectos cross-lagged
  ...
  - No se observa un efecto longitudinal intra-individuo de la justificación de brecha salarial sobre la participación en marchas (b = -0.01; SE= 0.005; p=0.09), ni de la participación en marchas sobre la percepción de brecha salarial (b = 0.017; SE=0.019; p=0.36)