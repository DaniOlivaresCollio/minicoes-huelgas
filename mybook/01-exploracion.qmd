...
#Paso 1
datos.clase <- data.frame((svytable(~class + ola + idencuesta, elsoc_diseno, round = F))) %>% dplyr::filter(Freq>0)  %>% group_by(ola) %>% mutate(porcentaje=Freq/sum(Freq)) %>% na.omit()

#Paso 2
etiquetas.clase <- data.frame((svytable(~class + ola, elsoc_diseno, round = F))) %>% group_by(ola) %>% mutate(porcentaje=Freq/sum(Freq)) %>% na.omit() %>% 
  mutate(idencuesta = 1)
datos.clase$class <- factor(datos.clase$class, levels = rev(levels(datos.clase$class)))
etiquetas.clase$class <- factor(etiquetas.clase$class, levels = rev(levels(etiquetas.clase$class)))
```

```{r alluvial}
colors<- c("#f1eef6ff","#bdc9e1ff","#b3b3b3ff","#74a9cfff","#0570b0ff")
alluvial_clase <- ggplot(datos.clase, aes(x = ola, fill = class, stratum = class,
...
data <- data %>% filter(perc_gerente>999)
data <- data %>% filter(perc_obrero>999)
data <- data %>% filter(just_gerente>999)
data <- data %>% filter(just_obrero>999)
summary(data$perc_gerente)
```
...
data$educacion <- car::recode(data$educ, "c(1,2,3)=1; c(4,5)=2; c(6,7,8,9,10)=3")

data$educacion <- set_labels(data$educacion,
...
reg3 <- lmer(brecha_perc~ola+marcha+class+(1|idencuesta), data=data)
reg4 <- lmer(brecha_perc~ola+marcha+class+sexo+edad+educacion+(1|idencuesta), data=data)

texreg::knitreg(list(reg1, reg2, reg3, reg4),
                custom.model.names=rep(c("Brecha percibida"),4))
```
...
reg3 <- lmer(brecha_just~ola+marcha+class+(1|idencuesta), data=data)
reg4 <- lmer(brecha_just~ola+marcha+class+sexo+edad+educacion+(1|idencuesta), data=data)

texreg::knitreg(list(reg1, reg2, reg3, reg4),
                custom.model.names=rep(c("Brecha justa"),4))
```
...
reg3 <- lmer(brecha_gerente~ola+marcha+class+(1|idencuesta), data=data)
reg4 <- lmer(brecha_gerente~ola+marcha+class+sexo+edad+educacion+(1|idencuesta), data=data)

texreg::knitreg(list(reg1, reg2, reg3, reg4),
                custom.model.names=rep(c("Brecha gerente"),4))
```
...
reg3 <- lmer(brecha_obrero~ola+marcha+class+(1|idencuesta), data=data)
reg4 <- lmer(brecha_obrero~ola+marcha+class+sexo+edad+educacion+(1|idencuesta), data=data)

texreg::knitreg(list(reg1, reg2, reg3, reg4),
                custom.model.names=rep(c("Brecha obrero"),4))
```
